...
export const HeaderGeneral: FC = () => {
  ...
  return (
    <>
      ...
      >
        <div className="absolute z-[1] top-0 left-0 right-0 h-[calc(var(--header-height)*2.1)] bg-gradient-to-b from-black from-0% via-[rgba(0,0,0,0.45)] via-45 to-transparent to-100%" />
        <div className="container relative z-[2] px-[calc(var(--interface-padding)+var(--interface-border-w)+1px)] md:border-none md:px-[var(--interface-corners-y-value)] flex flex-row items-start">
          {/* Angles for the sides */}
          <HeaderVectorInterface isShort={false} />
          ...
          <div className="flex flex-row flex-1 md:flex-initial">
            <HeaderRightBlock />
            <HeaderBlockSeparator className="hidden w-12 shrink-0 !border-r-0 md:block" />
          </div>
        </div>
      </header>
    </>
