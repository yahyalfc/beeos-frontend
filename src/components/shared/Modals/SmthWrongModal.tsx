...
interface SmthWrongModalProps {
  text?: string;
}

const SmthWrongModal: FC<SmthWrongModalProps> = ({
  text = "YOU'RE NOT IN THE GUARANTEED OR WAITLIST PHASES",
}) => {
  const { setParams, getParams } = useSearchQuery();
  const modalQuery = getParams(MODALS_QUERIES.SMTH_WRONG);
  let isOpen = false;
  ...
  return (
    <Dialog
      ...
      onOpenChange={(v) => {
        if (!v) {
          setParams(MODALS_QUERIES.SMTH_WRONG, "false");
        }
      }}
    >
          ...
          <div className="flex relative z-[2] flex-col justify-center items-center max-w-[358px]">
            ...
            <div className="shrink-0 flex justify-center items-center flex-1 w-full">
              <DefaultButton
                ...
                size="default"
                variant="accent"
                onClick={() => setParams(MODALS_QUERIES.SMTH_WRONG, "false")}
              >
                Got it
              </DefaultButton>
            </div>
