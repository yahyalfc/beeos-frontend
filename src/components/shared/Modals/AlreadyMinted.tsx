...
const AlreadyMintedSub: FC = () => {
  const { setParams, getParams } = useSearchQuery();
  const modalQuery = getParams(MODALS_QUERIES.ALREADY_MINTED);
  let isOpen = false;
  ...
  return (
    <Dialog
      ...
      onOpenChange={(v) => {
        if (!v) {
          setParams(MODALS_QUERIES.ALREADY_MINTED, "false");
        }
      }}
    >
        ...
        <DialogContentBody className="flex flex-col justify-center items-center">
          ...
          <div className="flex relative z-[2] flex-col justify-center items-center max-w-[358px]">
            ...
            <p className="mb-8 text-md text-center">
              Congrats on your new NFT! You can find it in your wallet.
            </p>
            <div className="shrink-0 flex justify-center items-center flex-1 w-full">
              <DefaultButton
                ...
                size="default"
                variant="accent"
                onClick={() => setParams(MODALS_QUERIES.ALREADY_MINTED, "false")}
              >
                Close
              </DefaultButton>
            </div>
...
export default AlreadyMintedSub;
