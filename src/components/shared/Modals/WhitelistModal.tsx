...
const WhitelistWalletSub: FC = () => {
  const { setParams, getParams } = useSearchQuery();
  const modalQuery = getParams(MODALS_QUERIES.WHITELIST_WALLET_MODAL);
  let isOpen = false;
  ...
  return (
    <Dialog
      ...
      onOpenChange={(v) => {
        if (!v) {
          setParams(MODALS_QUERIES.WHITELIST_WALLET_MODAL, "false");
        }
      }}
    >
        ...
        <DialogContentBody className="flex flex-col justify-center items-center">
          ...
          <div className="flex relative z-[2] flex-col justify-center items-center max-w-[358px]">
            ...
            <p className="mb-8 text-md text-center">
              Your wallet is on the GTD Whitelist! Get ready for guaranteed mint
              access.
            </p>
...
export default WhitelistWalletSub;
